...
const ContaPagarOmieCentralQueue = require("../../../services/fila/handlers/contaPagar/omie-central");
...
const listarTodas = async (req, res) => {
  try {
    ...
    const results = await IntegracaoContaPagarOmieCentral.find(filtros).sort({
      executadoEm: -1,
    });
...
const processar = async (req, res) => {
  try {
    ContaPagarOmieCentralQueue.start();
...
const arquivar = async (req, res) => {
  try {
    ...
    const ticketArquivado =
      await IntegracaoContaPagarOmieCentral.findByIdAndUpdate(id, {
        arquivado: true,
...
const reprocessar = async (req, res) => {
  try {
    ...
    const integracao = await IntegracaoContaPagarOmieCentral.findById(id);
    if (!integracao) {
      return res.status(404).json("Integração não encontrada");
    }

    const integracaoASerProcessada =
      await IntegracaoContaPagarOmieCentral.findOne({
...
const listarComPaginacao = async (req, res) => {
  try {
    ...
    const filterFromFiltros = filterUtils.queryFiltros({
      filtros: rest,
      schema: IntegracaoContaPagarOmieCentral.schema,
    });
    ...
    const [integracoes, totalIntegracoes] = await Promise.all([
      IntegracaoContaPagarOmieCentral.find(queryResult)
        .skip(skip)
        .limit(limite)
        .sort(sorting),
      IntegracaoContaPagarOmieCentral.countDocuments(queryResult),
    ]);
