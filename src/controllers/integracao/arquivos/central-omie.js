...
const listarTodas = async (req, res) => {
  try {
    ...
    const results = await IntegracaoArquivosCentralOmie.find(filtros).sort({
      executadoEm: -1,
    });
...
const processar = async (req, res) => {
  try {
    ArquivosCentralOmieQueue.start();
...
const arquivar = async (req, res) => {
  try {
    ...
    const ticketArquivado =
      await IntegracaoArquivosCentralOmie.findByIdAndUpdate(id, {
        arquivado: true,
...
const reprocessar = async (req, res) => {
  try {
    ...
    const integracao = await IntegracaoArquivosCentralOmie.findById(id);
    if (!integracao) {
...
const listarComPaginacao = async (req, res) => {
  try {
    ...
    const filterFromFiltros = filterUtils.queryFiltros({
      filtros: rest,
      schema: IntegracaoArquivosCentralOmie.schema,
    });
    ...
    const [integracoes, totalIntegracoes] = await Promise.all([
      IntegracaoArquivosCentralOmie.find(queryResult)
        .skip(skip)
        .limit(limite)
        .sort(sorting),
      IntegracaoArquivosCentralOmie.countDocuments(queryResult),
    ]);
